...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("Request your Self Bro 🕊️", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            return await query.answer('Are U Happy Now')
    ...
    if query.data.startswith("file"):
        ...
        try:
            msg = await client.send_cached_media(
                ...
                file_id=file_id,
                caption=f'<b>Hai 👋 {query.from_user.mention}</b> 😍\n\n<code>[@cineflix_group] {title}</code>\n\n⚠️ <i>This file will be deleted from here within 5 minute 𝗣𝗹𝗲𝗮𝘀𝗲 𝗦𝗮𝘃𝗲 𝗧𝗵𝗲 𝗙𝗶𝗹𝗲...⚠️ !!!</i>\n\n<i>{CUSTOM_FILE_CAPTION}\n\n<i><b> Powered by {query.message.chat.title}</b></i>',
                protect_content=True if ident == "filep" else False 
            )
            msg1 = await query.message.reply(
                f'<b> Hai 👋 {query.from_user.mention} </b>😍\n\n<b>📫 Your File is Ready</b>\n\n'           
                f'<b>📂 Fɪʟᴇ Nᴀᴍᴇ</b> : <code>[𝑪𝒊𝒏𝒆𝑭𝒍𝒊𝒙] {title}</code>\n\n'              
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton("⚠️ 𝖢𝖺𝗇'𝗍 𝖠𝖼𝖼𝖾𝗌𝗌 ❓ 𝖢𝗅𝗂𝖼𝗄 𝖧𝖾𝗋𝖾 ⚠️", url=f'https://t.me/+huGSYHQ4WGIwMjk1')
                        ]
                    ]
                )
            )
     ...
     [
        ...
        buttons = [[
        InlineKeyboardButton('ᴄɪɴᴇꜰʟɪx', url='https://t.me/cineflix_group'),
